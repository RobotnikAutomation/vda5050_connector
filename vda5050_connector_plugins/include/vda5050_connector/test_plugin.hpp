...
{
class TestPlugin : public GenericPlugin
{
    public:
        TestPlugin() = default;
        ~TestPlugin() = default;
        void init() override;
        void update(vda5050_msgs::msg::State& state) override;
        std::vector<SubscriptionFields> generateSubscriptionFields() override;

    private:
        void testSubscriptionCb(std::shared_ptr<rclcpp::SerializedMessage> msg);
        void testSubscriptionCb2(std::shared_ptr<rclcpp::SerializedMessage> msg);
};
